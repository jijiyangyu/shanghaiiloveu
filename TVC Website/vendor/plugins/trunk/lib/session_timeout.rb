  ...
  def check_session_expiry(time, opts)
    ...
    if session[:expires_at]
      if session_has_timed_out?
        unless opts[:before_timeout].nil?
          logger.info "::: Calling before timeout callback"
          opts[:before_timeout].call(self) if opts[:before_timeout].instance_of?(Proc)
          self.send(opts[:before_timeout]) if opts[:before_timeout].instance_of?(Symbol)
        end
